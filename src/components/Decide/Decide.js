...
function Card(props) {
  return (
    <div
      ...
      style={{
        backgroundColor: props.cardType === 'first' ? '#66C8EB' : '#E73130',
      }}>
      {props.showResult === true ? (
        <div>
          ...
          <div
            ...
            style={getPercentageColor(props.cardType)}>
            {props.percentage}
            {'%'}
          </div>
          <div className={agree}>
            {props.decision.selectedCount}
            {props.chosen === true
              ? props.decision.selectedCount === 1
                ? ' agrees'
                : ' agree'
              : ' disagree'}
          </div>
          <div
            className={decisionTextAnswered}
            style={{ fontSize: props.showResult === true ? 15 : 25 }}>
            {props.decision.text}
          </div>
        </div>
      ) : (
...
export default function Decide({
  ...
  const totalCount =
    decision.firstOption.selectedCount + decision.secondOption.selectedCount
    ...
    <div>
      ...
      <div className={container}>
        ...
        <Card
          cardType="first"
          chosen={
            noDecisionMade ? false : usersDecision.chosen === 'firstOption'
          }
          ...
          percentage={
            totalCount === 0
              ? 0
              : parseInt(
                  (decision.firstOption.selectedCount / totalCount) * 100,
                  10
                )
          }
          onSelect={() =>
            usersDecision && usersDecision.chosen === 'firstOption'
              ? null
        ...
        <Card
          cardType="second"
          chosen={
            noDecisionMade ? false : usersDecision.chosen === 'secondOption'
          }
          showResult={!noDecisionMade}
          ...
          percentage={
            totalCount === 0
              ? 0
              : parseInt(
                  (decision.secondOption.selectedCount / totalCount) * 100,
                  10
                )
          }
          onSelect={() =>
            usersDecision && usersDecision.chosen === 'secondOption'
              ? null
              : onSelect('secondOption')
          }
        />
      </div>
    </div>
